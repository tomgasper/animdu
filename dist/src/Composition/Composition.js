...
export class Composition {
        ...
        if (obj.length && obj.length > 1) {
            obj.forEach((obj) => {
                if (obj && obj instanceof RenderableObject) {
                    ...
                    if (!obj.parent)
                        obj.setParent(this.viewport);
                }
            });
        }
        else if (obj instanceof RenderableObject)
            this.objects.push(obj);
    }
    removeObj(obj) {
    ...
    findStartComponents() {
        ...
        let findStart = (c) => {
            ...
            if (handleL.line.connection.connectedObj.node instanceof ObjNode)
                return c;
        ...
        activeViewer.components.forEach((component) => {
            ...
            if (componentStart && !compsToProcess.has(componentStart))
                compsToProcess.add(componentStart);
        });
            ...
            componentNodes.FNC.forEach((node) => {
                ...
                paramsOUT.forEach((param, indx) => {
                    ...
                    beforeParams.forEach((beforeParam) => {
                        if (beforeParam.name == param.name)
                            beforeVal = beforeParam.value;
                    });
                    let paramToAnim = {
                        name: param.name,
