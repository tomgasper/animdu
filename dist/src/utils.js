...
export function getProjectionMat(gl) {
    if (!gl || !gl.canvas)
        throw Error("Invalid gl/canvas object");
...
export function renderObject(gl, obj, program) {
    ...
    if (!(obj instanceof RenderableObject))
        throw Error("Object is not renderable!");
...
const checkIfMatrix = (mat) => {
    if (mat.length !== 9 || !Array.isArray(mat))
        return false;
    return true;
};
export const transformToParentSpace = (parent, vecArr, unscale = true, cameraMat) => {
    ...
    if (cameraMat)
        m3.multiplyInPlace(invMat, cameraMat, invMat);
    if (unscale) {
