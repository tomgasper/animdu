...
export const initInputListeners = (app) => {
    ...
    app.document.addEventListener("keydown", (e) => {
        if (app.inputState.keyPressed.indexOf(e.key) === -1)
            app.inputState.keyPressed.push(e.key);
    ...
    app.gl.canvas.addEventListener("wheel", (e) => {
        ...
        let compCamera;
        if (clipY > 0)
            compCamera = app.activeComp.camera;
        else
            compCamera = app.UI.viewer.camera;
        ...
        const [preZoomX, preZoomY] = m3.transformPoint(m3.inverse(viewProjectionMat), [clipX, clipY]);
        ...
        const [postZoomX, postZoomY] = m3.transformPoint(m3.inverse(viewProjectionMat), [clipX, clipY]);
