...
export const getPosFromMat = (obj) => {
    ...
    if (obj instanceof GeometryObject || obj.length == 9) {
        ...
        throw Error("Wrong input object!");
};
export const moveObjectWithCoursor = (app) => {
    ...
    */
    ...
    if (objToDrag.moveRestriction) {
        ...
        if (newPos[0] < xBound[0] || newPos[0] > xBound[1])
            newPos[0] = objToDrag.properties.position[0];
        if (newPos[1] < yBound[0] || newPos[1] > yBound[1])
            newPos[1] = objToDrag.properties.position[1];
    }
...
export const canMoveObj = (app) => {
    ...
    if (app.isMouseDown && objToMove.properties.movable === true)
        return true;
    else
        return false;
};
...
export const addObjToDrawList = (obj, drawList, camera) => {
    ...
    if (!(obj instanceof RenderableObject))
        throw new Error("Incorrect object pushed to draw list!");
...
const setActiveComp = (appRef, comp) => {
    if (comp && comp instanceof Composition)
        appRef.activeComp = comp;
    else
        console.log("Trying to set incorrect composition as active!");
};
