    ...
    if (typeof programInfo !== "undefined") // this is drawing for picking pass, with specified shader
     {
        ...
        objsToDraw.forEach((obj, i) => {
            if (!(obj instanceof RenderableObject))
                throw Error("Incorrect object in draw loop!" + obj);
            ...
            if (obj.properties.resolution)
                obj.properties.resolution = [app.gl.canvas.width, app.gl.canvas.height];
            if (!obj.parent)
            ...
            if (obj instanceof UINodeEditor || obj instanceof UISceneViewport)
                obj.setProjectionAndCalcFinalTransform(getProjectionMat(app.gl));
            else
                obj.setProjectionAndCalcFinalTransform(viewProjection);
        ...
        objsToDraw.forEach((obj, ii) => {
            ...
            if (obj.properties.blending === true) {
                if (!isBlendingEnabled)
                    isBlendingEnabled = app.setBlendingEnabled(true);
...
//# sourceMappingURL=AppDraw.js.map